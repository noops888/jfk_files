...
IdAid. This issue is not a new one, it has been discussed before with as NARA. Aifé data recorded on
‘the Id Aid reflects the. tats of the IdAid and/or the version of the-document currently at WARK - namely
all are

Unclassified. The data is in our file, however, since it was not a required rapes
field at input there may be instances where the field is blank. dows
...
provided. | have talked toCynde\about this and she has checked the
...
3 ition status field. (Cyndajnforms me that the program inserts a
a rey Refarzajdmarking when the document originator is anything but CIA, This
jarking was not een by the indexers but by the program. The theory
...
release included ldAids for those documents pending coordination - the
\dAids appropriately stamped with “Pending Coordination” - but no
...
documents would be inter-filed in the basic file. an
...
Board gets ready to lock at documents the “trailing” coordinations will be
complete.

Concern 4: Carriage returns in the Comments field ee qe wv % -c loan “Pe
...
Recommendation: Program to remove the carriage returns when thay 3
